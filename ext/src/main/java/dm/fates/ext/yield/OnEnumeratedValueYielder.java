...
import dm.fates.ext.yield.OnEnumeratedValueYielder.YielderStack;
 ...
 */
class OnEnumeratedValueYielder<V, R> implements LoopYielder<YielderStack, V, R>, Serializable {

  private static final long serialVersionUID = BuildConfig.VERSION_HASH_CODE;

  private final TriMapper<? super Long, ? super V, ? super YieldOutputs<R>, Boolean> mMapper;

  OnEnumeratedValueYielder(
      @NotNull final TriMapper<? super Long, ? super V, ? super YieldOutputs<R>, Boolean> mapper) {
    mMapper = ConstantConditions.notNull("mapper", mapper);
  }

  public void done(final YielderStack stack, @NotNull final YieldOutputs<R> outputs) {
  }
  ...
  public YielderStack value(final YielderStack stack, final V value,
    ...
    final Boolean isLoop = mMapper.apply(stack.count, value, outputs);
    ++stack.count;
    if (isLoop != null) {
      stack.isLoop = isLoop;
    }
  ...
  private static class YielderProxy<V, R> extends SerializableProxy {
    ...
    private YielderProxy(
        final TriMapper<? super Long, ? super V, ? super YieldOutputs<R>, Boolean> mapper) {
      super(proxy(mapper));
    }
    ...
    private Object readResolve() throws ObjectStreamException {
      try {
        ...
        return new OnEnumeratedValueYielder<V, R>(
            (TriMapper<? super Long, ? super V, ? super YieldOutputs<R>, Boolean>) args[0]);

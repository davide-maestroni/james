...
import dm.fates.ext.join.ZipStrictJoiner.JoinerStack;
import dm.fates.util.DoubleQueue;

/**
 * Created by davide-maestroni on 02/16/2018.
 */
class ZipStrictJoiner<V> implements LoopJoiner<JoinerStack<V>, V, List<V>>, Serializable {

  private static final ZipStrictJoiner<?> sInstance = new ZipStrictJoiner<Object>();

  private static final long serialVersionUID = BuildConfig.VERSION_HASH_CODE;

  private ZipStrictJoiner() {
  }

  @NotNull
  @SuppressWarnings("unchecked")
  static <V> ZipStrictJoiner<V> instance() {
    return (ZipStrictJoiner<V>) sInstance;
  }
  ...
  public void settle(final JoinerStack<V> stack,
    ...
    if (stack.failure == null) {
      ...
      for (final DoubleQueue<V> state : stack.states) {
        if (!state.isEmpty()) {
          hasValues = true;
          break;
        }
      ...
      if (hasValues) {
        evaluation.addFailure(new IllegalArgumentException("loop has unmatched values"));
      }

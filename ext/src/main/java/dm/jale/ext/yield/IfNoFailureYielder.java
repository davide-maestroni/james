 ...
 */
class IfNoFailureYielder<V> implements LoopYielder<Boolean, V, V>, Serializable {

  private static final long serialVersionUID = BuildConfig.VERSION_HASH_CODE;

  private final Observer<? super YieldOutputs<V>> mObserver;

  IfNoFailureYielder(@NotNull final Observer<? super YieldOutputs<V>> observer) {
  ...
  private static class YielderProxy<V> extends SerializableProxy {
    ...
    private Object readResolve() throws ObjectStreamException {
      try {
        ...
        return new IfNoFailureYielder<V>((Observer<? super YieldOutputs<V>>) args[0]);

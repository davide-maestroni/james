...
import dm.jale.ext.ConcatJoiner.CombinerStack;
import dm.jale.ext.config.BuildConfig;

/**
 * Created by davide-maestroni on 02/16/2018.
 */
class ConcatJoiner<V> implements LoopJoiner<CombinerStack<V>, V, V>, Serializable {
  ...
  public CombinerStack<V> init(@NotNull final List<Loop<V>> contexts) {
  ...
  @SuppressWarnings("unchecked")
  public CombinerStack<V> value(final CombinerStack<V> stack, final V value,
      @NotNull final EvaluationCollection<V> evaluation, @NotNull final List<Loop<V>> contexts,
      final int index) {
    if (stack.index == index) {
